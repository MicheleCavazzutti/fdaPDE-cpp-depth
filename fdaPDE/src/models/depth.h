...
namespace fdapde {
  namespace models {
	...
	for (auto i=0; i<n_nodes; i++){
	  ...
	  if(this->voronoi_.local_dim == 2 && this->voronoi_.embed_dim == 3 ){ // resort to external measures for 2.5D mesh
	    measure  = this->external_voronoi_measures_[i];
	  }
	  
	  if(this->voronoi_.local_dim == 3 && this->voronoi_.embed_dim == 3 ){ // resort to external measures for 3D mesh
	    measure  = this->external_voronoi_measures_[i];
	  }

...
namespace fdapde {
  namespace models {
    ...
    public:
      ...
      const Voronoi<SpaceDomainType> & voronoi() const { return voronoi_; } // Returns the voronoi tessellation used by the method, from which also locations, cells and measures can be accessed
      ...
      void init() { // Initialization routine, prepares the environment for the solution of the problem.
        ...
        Voronoi<SpaceDomainType> voronoi(domain_); // In future we will need to understand whether to store this object this way or not, maybe in initializztion of the problem
	...
	for (sdt::size_t i; i<voronoi_.n_cells(); i++){// for each node of the mesh, count how many times a cell has been observed in the Voronoi mask. 
	...
	//BinaryMatrix<fdapde::Dynamic> voronoi_mask;
	...
	for (std::size_t i=0; i<n_nodes; i++){
	  ...
	  Real measure = this->voronoi_.cell(i)).measure();
    ...
    private:
      ...
      Voronoi<SpaceDomainType> voronoi_;                // Voronoi representation of the domain, set in init. This component is only local, and needs to be revised when modifications occur in syntax
      ...
      BinaryMatrix<fdapde::Dynamic> Voronoi_NA_fit_;    // Binary matrix containing the missing data pattern for the voronoi representation of the train functions. Dimension n_tain x n_nodes
      DMatrix<double> voronoi_r_pred_; 			// Voronoi values for the predict functions. Dimension: n_predict x n_nodes
      BinaryMatrix<fdapde::Dynamic> Voronoi_NA_pred_;   // Binary matrix containing the missing data pattern for the voronoi representation of the pred functions. Dimension n_tain x n_nodes
